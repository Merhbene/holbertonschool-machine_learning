...
def autoencoder(input_dims, hidden_layers, latent_dims):
    ...
    def total_loss(x, x_decoded):
        ...
        kl_loss = -0.5 * keras.backend.sum(1 + log_sig - keras.backend.square(mu) - keras.backend.exp(log_sig), axis=1)
        #keras.backend.exp(log_sig) + keras.backend.square(mu) - 1 - log_sig
